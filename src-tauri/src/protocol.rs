...
//! on stderr. LITTLECLAUDE responds via stdin with `control_response` messages.
//!
//! Note: Permission responses (LITTLECLAUDE → CLI) are built as raw `serde_json::json!`
...
// ─── LITTLECLAUDE → CLI: SDK control requests (stdin) ─────────────────────────
...
/// SDK control request subtypes that LITTLECLAUDE can send to CLI.
#[derive(Debug, Serialize)]
#[serde(tag = "subtype")]
