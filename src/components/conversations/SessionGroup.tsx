...
export const SessionGroup = memo(function SessionGroup({
  ...
  const { pinnedItems, dateGroups } = useMemo(() => {
    ...
    // Split "earlier" group into month subgroups
    const earlierItems = categoryMap.get('earlier');
    if (earlierItems && earlierItems.length > 0) {
      const monthMap = new Map<string, SessionListItem[]>();
      for (const s of earlierItems) {
