 ...
 */
export const QuestionCard = memo(function QuestionCard({ message, floating }: Props) {
  const t = useT();
  ...
  const handleConfirm = useCallback(async () => {
    ...
    if (isLast) {
      ...
      if (!stdinId) return;
      // CLI 的 AskUserQuestion 工具按【问题文本】匹配答案（SDK 约定：
      // updatedInput.answers = { "问题文本": "所选项" }）。用索引当 key 时工具
      // 匹配不到任何问题，答案全部丢弃 —— 模型侧看到的就是"选了也为空"。
      const answers: Record<string, string> = {};
      questions.forEach((q, qIdx) => {
        const key = q.question?.trim() || String(qIdx);
        if (useOther[qIdx] && otherText[qIdx]?.trim()) {
          answers[key] = otherText[qIdx].trim();
        } else {
          const selected = selectedMap[qIdx] || new Set<number>();
