...
import { getCachedThumbnail, type FileAttachment } from '../../hooks/useFileAttachments';
...
export const FileUploadChips = memo(function FileUploadChips({ files, onRemove, isProcessing }: FileUploadChipsProps) {
  const t = useT();
    ...
    <div className="flex items-center gap-1.5 px-2 py-2
      ...
      {files.map((file) => (
        ...
        >
          ...
          {file.isImage && getCachedThumbnail(file.path) ? (
            <img
              src={getCachedThumbnail(file.path)}
              alt={file.name}
