...
export function McpTab() {
  ...
  const handleImport = useCallback(async (names?: string[]) => {
    try {
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            title={t('mcp.scanTitle')}
          >
            {isScanning ? t('mcp.scanning') : t('mcp.scanLocal')}
          </button>
          {missingDiscovered.length > 0 && (
            ...
            >
              {t('mcp.importCount', { count: String(missingDiscovered.length) })}
            </button>
          )}
          <button
      ...
      {discoveredServers.length > 0 && (
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <span className="text-xs font-medium text-text-muted">{t('mcp.scanResults')}</span>
            <span className="text-xs text-text-tertiary">
              {missingDiscovered.length > 0
                ? t('mcp.nNotImported', { count: String(missingDiscovered.length) })
                : t('mcp.allImported')}
            </span>
          </div>
          <div className="space-y-2 max-h-56 overflow-y-auto pr-1">
            {discoveredServers.map((server) => (
              <DiscoveredMcpCard
                ...
                server={server}
                onImport={() => handleImport([server.name])}
              />
            ))}
          </div>
        </div>
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2">
        ...
        >
          {server.imported ? t('mcp.imported') : t('mcp.notImported')}
        </span>
        {!server.imported && (
          <button
            onClick={onImport}
      ...
      {envCount > 0 && (
        <p className="mt-0.5 text-[11px] text-text-tertiary">{envCount} {t('mcp.envCount')}</p>
      )}
    </div>
  );
}
