...
export function ProviderManager({ alwaysExpanded = false }: { alwaysExpanded?: boolean } = {}) {
  ...
  const activeProviderPerBackend = useProviderStore((s) => s.activeProviderPerBackend);
  ...
  const activeProviderId = activeProviderPerBackend[activeBackend] ?? null;
  const activeProvider = activeProviderId ? providers.find((p) => p.id === activeProviderId) : undefined;
  const activeLabel = activeProvider ? activeProvider.name : t('provider.inherit');

  // Filter providers by the selected backend tab:
  // - Providers without cliBackend default to 'claude' (legacy migration)
  // - Providers with explicit cliBackend only appear in their matching tab
  const filteredProviders = useMemo(() => {
    return providers.filter((p) => {
      const backend = p.cliBackend || 'claude';
      return backend === activeBackend;
    });
  }, [providers, activeBackend]);
  ...
  const handleAddFromPreset = useCallback((preset: PresetProvider) => {
    ...
    const presetBackend = preset.cliBackend || 'claude';

    // Auto-switch to the preset's backend if we're on a different tab
    if (presetBackend !== activeBackend) {
      setActiveBackend(presetBackend);
    }
    ...
    addProvider({
      ...
      modelMappings: [
        { tier: 'opus', providerModel: preset.defaultModels?.opus || preset.defaultModel || '' },
        { tier: 'sonnet', providerModel: preset.defaultModels?.sonnet || preset.defaultModel || '' },
        { tier: 'haiku', providerModel: preset.defaultModels?.haiku || preset.defaultModel || '' },
      ],
      extra_env: { ...preset.extra_env },
      preset: preset.id,
      cliBackend: preset.cliBackend,
    });
    const { providers: updated } = useProviderStore.getState();
    const last = updated[updated.length - 1];
    if (last) {
      setActive(last.id, presetBackend);
      setEditingId(last.id);
    }
  }, [addProvider, providers, setActive, activeBackend]);

  const handleAddCustom = useCallback(() => {
    addProvider({
      name: '',
      baseUrl: '',
      apiFormat: activeBackend === 'codex' ? 'openai' : 'anthropic',
      modelMappings: [
      ...
      extra_env: {},
      cliBackend: activeBackend,
    });
    const { providers: updated } = useProviderStore.getState();
    const last = updated[updated.length - 1];
    if (last) {
      setActive(last.id, activeBackend);
      setEditingId(last.id);
    }
  }, [addProvider, setActive, activeBackend]);
  ...
  const handleImport = useCallback(async () => {
    ...
    try {
      ...
      const last = updated[updated.length - 1];
      if (last) {
        const importedBackend = last.cliBackend || 'claude';
        if (importedBackend !== activeBackend) {
          setActiveBackend(importedBackend);
        }
        setActive(last.id, importedBackend);
        setEditingId(last.id);
      }
    } catch (e) {
    ...
    <div className={alwaysExpanded ? '' : 'pt-2 border-t border-border-subtle'}>
      ...
      {isExpanded && (
        <div className="space-y-3 ml-0">
          ...
          <div className="flex gap-1 p-0.5 rounded-lg bg-bg-secondary border border-border-subtle">
            {(['claude', 'codex'] as const).map((backend) => {
              ...
              const backendProvider = backendProviderId
                ...
                : null;
              const backendLabel = backendProvider
              ...
              return (
                <button
                  ...
                  className={`flex-1 px-2 py-1 rounded text-xs font-medium transition-smooth
          ...
          >
            <button
              onClick={() => setActive(null, activeBackend)}
