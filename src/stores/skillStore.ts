...
/** Current working directory, passed to skill CRUD for project-path validation. */
function skillCwd(): string | null {
  return useSettingsStore.getState().workingDirectory || null;
}
...
export const useSkillStore = create<SkillState>()((set, get) => ({
  ...
  selectSkill: async (skill: SkillInfo) => {
    ...
    try {
      const content = await bridge.readSkill(skill.path, skillCwd());
  ...
  saveSkill: async () => {
    ...
    set({ isSaving: true });
    try {
      await bridge.writeSkill(selectedSkill.path, editContent, skillCwd());
      set({ skillContent: editContent, editContent: null, isSaving: false });
    } catch (e) {
      console.error('Failed to save skill:', e);
      // A10: a failed save silently discards the user's edits — surface it.
  ...
  deleteSkill: async (skill: SkillInfo) => {
    try {
      await bridge.deleteSkill(skill.path, skillCwd());
      ...
      showToast(t('skills.deleteFailed'), 'error');
    }
  },
  ...
  createSkill: async (name: string, scope: 'global' | 'project', cwd: string, content: string) => {
    ...
    try {
      await bridge.writeSkill(basePath, content, cwd);
      // Refresh the list
      await get().fetchSkills(cwd);
    } catch (e) {
      console.error('Failed to create skill:', e);
      showToast(t('skills.createFailed'), 'error');
    }
  },
  ...
  toggleEnabled: async (skill: SkillInfo) => {
    ...
    try {
      await bridge.toggleSkillEnabled(skill.path, newEnabled, skillCwd());
      ...
      // A10: the optimistic flip already updated the UI — tell the user the
      // change did NOT stick, otherwise they'll believe it did.
      showToast(t('skills.toggleFailed'), 'error');
    }
  },
}));
