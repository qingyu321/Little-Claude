...
All notable changes to Little Claude will be documented in this file.

本文件记录 Little Claude 的所有重要更新。
...
- **DeepSeek 模型选择修复** — 选其它模型不再落到默认 v4-flash：模型选择器在 DeepSeek 后端直读 DSH 真实 catalog（`llm.models` 免 session，分组 + reasoning 档位），可直接选 qwen3.7-max / glm-5.2 / kimi-k3 / grok-4.5 等全部模型并经 `session.selectModel` 钉住（活服务实测：catalog id 生效、非 catalog id 被拒落回默认）
- **DeepSeek 权限模式对齐（P-Per）** — 全自动→`danger-full-access`（从不询问 + 全权沙箱）、计划→`read-only`（只读防误写；DSH 真 plan 协作无 RPC 可及取安全近似）、标准自动/询问→`workspace-write`（逐工具询问）；模式切换即时 `settings.mutate` 同步，新建 DSH 会话继承，已在跑的会话保留其钉死权限
- **⚡ 徽章容错** — DSH 回合即使无流式文本 token 到达计速器（快速回合/通道抖动），回合末仍用底层真值 pin 出「首token xx · 吐字 xx tok/s」；徽章可见门补齐 firstToken/decode 字段

> 含此前 v1.2.0 覆盖发布已落地的 DSH 收尾修复（实时链路、agent 预设、思考链透传、首 token 计速底层真值、GoalBar 照抄 DSH、全局圆角软化、排队窗、TodoDock、token 统计/口径）。
...
- **Skip global MCP servers for faster cold start** — CLI now starts with `--strict-mcp-config`, bypassing all MCP servers configured in `~/.claude.json`. This eliminates 20-30s of MCP initialization overhead on first response. Little Claude users don't need MCP servers (chrome-devtools, codex, gemini, etc.) — those are developer tools, not chat client features.
...
- **AI reply avatar** — Chat avatar changed from accent-colored "C" letter to the Little Claude `</>` logo SVG. Adapts to light/dark mode and all color themes via `--color-icon-slash` CSS variable.
...
- **AI 回复头像** — 聊天头像从 accent 色 "C" 字母改为 Little Claude `</>` logo SVG。通过 `--color-icon-slash` CSS 变量自动适配亮/暗模式和所有主题色。
...
- **CLI Settings Override (Critical)** — Claude CLI's own `~/.claude/settings.json` `env` section could silently override Little Claude's injected `ANTHROPIC_BASE_URL` and `ANTHROPIC_AUTH_TOKEN`, causing all API requests to route to the wrong endpoint regardless of user configuration. Little Claude now automatically strips conflicting `ANTHROPIC_*` entries from settings.json when in custom/official mode, and clears inherited shell environment variables that aren't explicitly set.
...
- **CLI 配置覆盖（关键修复）** — Claude CLI 自身的 `~/.claude/settings.json` 中的 `env` 字段会静默覆盖 Little Claude 注入的 `ANTHROPIC_BASE_URL` 和 `ANTHROPIC_AUTH_TOKEN`，导致无论用户如何配置，所有 API 请求都被路由到错误的端点。Little Claude 现在在 custom/official 模式下自动清理 settings.json 中冲突的 `ANTHROPIC_*` 条目，并清除子进程继承的 shell 环境变量。
...
- **Windows Git Bash Auto-Install** — On first CLI installation on Windows, Little Claude now automatically downloads and deploys PortableGit (v2.47.1.2). Users no longer need to manually install Git for Windows. The `bash.exe` is deployed to the app-local `git/` directory.
...
- **Windows Git Bash 自动安装** — 在 Windows 上首次安装 CLI 时，Little Claude 自动下载并部署 PortableGit (v2.47.1.2)。用户无需手动安装 Git for Windows。`bash.exe` 部署到应用本地 `git/` 目录。
...
- **Node.js Local Deployment** — On first launch, if the system has no Node.js/npm, Little Claude automatically downloads Node.js LTS (v22) and deploys it to the app-local directory. No sudo/admin permissions required. Supports all 6 platforms (macOS arm64/x64, Windows x64, Linux x64/arm64).
...
- **Node.js 本地部署** — 首次启动时若系统无 Node.js/npm，Little Claude 自动下载 Node.js LTS (v22) 并部署到应用本地目录。无需 sudo/管理员权限。支持全部 6 个平台（macOS arm64/x64、Windows x64、Linux x64/arm64）。
...
- **Windows/macOS CLI Path Fix** — `open_terminal_login` and `start_claude_login` now use enriched PATH and proper error handling instead of falling back to bare `claude.cmd`. Little Claude manages the CLI path internally — users never need terminal access.
...
- **Windows/macOS CLI 路径修复** — `open_terminal_login` 和 `start_claude_login` 改用 enriched PATH 和正确的错误处理。Little Claude 内部管理 CLI 路径，用户无需终端操作。
...
Little Claude now automatically detects whether the Claude Code CLI is installed when launched for the first time. If the CLI is missing or the user is not logged in, a step-by-step setup wizard guides them through the entire process — no terminal required.
...
Little Claude 现在会在首次启动时自动检测系统中是否已安装 Claude Code CLI。如果未安装或未登录，应用会通过分步引导向导帮助用户完成整个设置过程，无需打开终端。
...
- Initial release of Little Claude — a beautiful desktop GUI for Claude Code
...
- Little Claude 首发 — Claude Code 的桌面 GUI 客户端
- 基于 NDJSON 流式传输的聊天界面
- 文件浏览器，支持预览和编辑
- 会话管理（创建、恢复、删除、导出）
- 快照/回退系统，支持代码回滚
